...
### لاگ اضطراری (Emergency Log)

در صورت بروز خطاهای حیاتی در هنگام بارگذاری افزونه، اطلاعات خطا در فایل زیر ذخیره می‌شود:

```
wp-content/homa-emergency-log.txt
```

این لاگ شامل اطلاعات دقیق خطاها می‌باشد و می‌تواند در عیب‌یابی مشکلات بسیار مفید باشد.

### مشکلات شناخته شده

#### خطای "WooCommerce not active"

این یک هشدار است و افزونه همچنان کار می‌کند، اما برخی ویژگی‌های مربوط به فروشگاه غیرفعال خواهند بود.

**راه حل**: نصب و فعال‌سازی افزونه WooCommerce

#### حلقه بی‌نهایت در Observer

افزونه دارای مکانیزم محافظتی در برابر حلقه‌های بی‌نهایت است. در صورت مشاهده رفتار غیرعادی، لاگ اضطراری را بررسی کنید.

#### خطای Output Buffer

مکانیزم ترجمه فقط در صفحات فرانت‌اند و پس از بارگذاری کامل WordPress فعال می‌شود. در صورت مشکل، کوکی `homa_translate_to` را پاک کنید.
